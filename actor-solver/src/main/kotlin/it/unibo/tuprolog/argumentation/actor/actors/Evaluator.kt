...
class Evaluator private constructor(
    ...
    private val master: ActorRef<KbMessage>,
            ...
            .onMessage(Add::class.java) { command ->
                ...
                result.forEach {
                    val query =
                        ActiveQuery(
                            0,
                            "query_${Random.nextInt(0, Int.MAX_VALUE)}",
                            -1,
                            command.elem,
                            it.toString(),
                            master,
    ...
    companion object {
        fun create(
            ...
            entityId: String,
        ): Behavior<KbMessage> = Behaviors.setup { context -> Evaluator(context, master, entityId) }
    }
}
